...
for epoch in range(NUM_EPOCHS):
    ...
    for batch_idx, (images, captions) in train_loader_tqdm:
        ...
        targets = captions[:, 1:]
        # Garantir alinhamento de seq_len
        min_seq_len = min(outputs.size(1), targets.size(1))  # menor seq_len
        outputs = outputs[:, :min_seq_len, :]  # corta outputs
        targets = targets[:, :min_seq_len]  # corta targets

        # Achatar tensores para cálculo da loss
